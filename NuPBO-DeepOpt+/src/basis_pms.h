...
public:
	...
	/*fps*/
	double *score_baocun;
	double *sscore_baocun;
	double *hhscore_baocun;
	double *score2;
	double *sscore2;
	double *hhscore2;
	int *goodvar_stack2;
	...
	/*NuPBO*/
	double *tune_soft_clause_weight;		 // soft-smooth
	double *tuned_degree_unit_weight = NULL; // hard-smooth
	...
	double lambda;
	int backward_step; // reward delay steps para d
	int NumSample;
	...
	void flip_fps(int flipvar); // fps
	void update_goodvarstack1(int flipvar);
	void update_goodvarstack2(int flipvar);
	void update_goodvarstack12(int flipvar); // fps
	int pick_var();
	void pick_vars(); // fps
	void settings();
	void turb();								 // turb
	int turb_pick_var(int last_flip_var);		 // turb
	void init_turb();							 // turb
	void flip_with_neighbor(int flipvar);		 // Nupbo
	void flip_update_score_multi(int flipvar);	 // Nupbo
	int pick_softc();							 // MAB
	void update_clause_scores(long long s);		 // MAB
	int pick_hardc();							 // MAB-h
	void update_clause_scores_hard(long long s); // MSB-h
